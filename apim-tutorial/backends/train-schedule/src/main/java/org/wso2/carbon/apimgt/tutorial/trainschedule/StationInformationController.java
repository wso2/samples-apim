...
public class StationInformationController {
    ...
    StationInformationManager manager = new StationInformationManager();
    ...
    public ResponseEntity<List<Train>> getTrainsForStation(@PathVariable("station_id") String stationId) {
        
        List<Train> stationTrains = manager.getTrains(stationId);
        return ResponseEntity.ok(stationTrains);
    }

    // POST /stations/{station_id}/trains
    @PostMapping("/stations/{station_id}/trains")
    public ResponseEntity<Train> createTrain(@PathVariable("station_id") String stationId, @RequestBody Train train) {
