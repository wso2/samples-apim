...
client_credentials=$(curl -k -u admin@railco.com:admin -H "Content-Type: application/json" -d "$(client_request)" https://$apim:$port/client-registration/v0.17/register| jq -r '.clientId + ":" + .clientSecret')
...
create_app_and_subscribe(){
    local application_id=$(curl -k -H "Authorization: Bearer $tom_access_token" -H "Content-Type: application/json" -X POST -d '{"name":"KeyCloakAPP","throttlingPolicy":"Unlimited","description":"","tokenType":"JWT","groups":null,"attributes":{}}' https://$apim:$port/api/am/devportal/applications | jq -r '.applicationId')
    local sub_id="$(curl -k -H "Authorization: Bearer $tom_access_token" -H "Content-Type: application/json" -X POST -d '{"apiId":"'$api_id'","applicationId":"'$application_id'","throttlingPolicy":"Unlimited"}' https://$apim:$port/api/am/devportal/subscriptions)"
}
