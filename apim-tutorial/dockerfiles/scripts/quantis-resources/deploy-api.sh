...
client_credentials=$(curl -k -u admin@quantis.com:admin -H "Content-Type: application/json" -d "$(client_request)" https://$apim:$port/client-registration/v0.17/register| jq -r '.clientId + ":" + .clientSecret')
...
vhost=$(get_vhost)
rate_and_comment(){
    local rating_id=$(curl -k -H "Authorization: Bearer $1" -H "Content-Type: application/json" -X PUT -d '{"rating":'$3'}' https://$apim:$port/api/am/devportal/apis/$2/user-rating | jq -r '.ratingId')
    local comment_id=$(curl -k -H "Authorization: Bearer $1" -H "Content-Type: application/json" -X POST -d @$4.json https://$apim:$port/api/am/devportal/apis/$2/comments | jq -r '.id')
    
}
rate_and_comment_and_reply(){
    local rating_id=$(curl -k -H "Authorization: Bearer $1" -H "Content-Type: application/json" -X PUT -d '{"rating":'$3'}' https://$apim:$port/api/am/devportal/apis/$2/user-rating | jq -r '.ratingId')
    local comment_id=$(curl -k -H "Authorization: Bearer $1" -H "Content-Type: application/json" -X POST -d @$4.json https://$apim:$port/api/am/devportal/apis/$2/comments | jq -r '.id')
    local reply=$(curl -k -H "Authorization: Bearer $5" -H "Content-Type: application/json" -X POST -d'{"content":"Thanks 😊","category":"general"}' https://$apim:$port/api/am/devportal/apis/$2/comments?replyTo=$comment_id | jq -r '.id')
...
create_and_publish_train_location_api() {
    ...
    local api_id=$(curl -k -H "Authorization: Bearer $pub_access_token" -H "Content-Type: application/json" -X POST -d @data.json https://$apim:$port/api/am/publisher/apis | jq -r '.id')
    local asyncapidefintion=$(curl -k -H "Authorization: Bearer $pub_access_token" -H "multipart/form-data" -X PUT -F apiDefinition=@asyncapi.json https://$apim:$port/api/am/publisher/apis/${api_id}/asyncapi | jq -r '.id')
    local rev_id=$( curl -k -H "Authorization: Bearer $pub_access_token" -H "Content-Type: application/json" -X POST -d '{"description": "first revision"}' https://$apim:$port/api/am/publisher/apis/${api_id}/revisions | jq -r '.id')
